...
### 3.6 [未解] OpenGL 後端:疊層畫下去之後,游標整個不見

**狀態:已重現、已縮小範圍、尚未修復。** 繞道是把圖形模式換成 SurfaceSDL
(`--gfx-mode=surfacesdl`);macOS 預設走 OpenGL,所以 macOS 玩家會直接撞到。
...
也就是說:先畫的疊層讓**後**畫的游標完全沒有輸出 —— 不是被蓋住、也不是被透明度混掉。

> 兩個方法論教訓:
> 1. **A/B 差異比對要挑對背景**。用「移動滑鼠到兩個位置各截一張、比對差異」判斷游標有沒有畫出來很好用,
>    但把兩個位置都選在黑色文字框上,再加上「關掉 blend」的變因,黑游標疊黑底就測不出差異 —— 訊號會自相矛盾。
> 2. **訊號開始互相矛盾時就停手**。連續二分到這裡已經出現前後不一致的結論(一度誤判「游標位置卡住」,
>    其實只是探針設了 `if (n < 40)` 只印前 40 幀、全是早期畫面)。這時候再多切幾刀不會更接近答案,
>    該換方法(讀 `Surface` / `Pipeline` 的貼圖建立與綁定路徑)而不是繼續猜。
