...
字串取得函式不一定只供畫面顯示。AGOS 的 `getStringPtrByID()` 同時被腳本條件比較
與內部存檔名稱使用；在共用出口無條件套用譯表，會讓翻譯改變遊戲狀態。

Elvira 1 的實例是英文 `The soldier`／`the soldier` 經不分大小寫比較原本相等，
繁中「那名士兵」／「士兵」卻不同。敵人分類失敗後戰鬥圖片變數保持 0，最後誤載
`002.VGA`。正解是顯示呼叫預設翻譯，語意比較、資源名稱、存檔名稱與鍵值明確取得
原始字串；不可把特定譯文改成相同來迎合程式比較。完整檢查表見
[`LOCALIZATION_SEMANTIC_ISOLATION.md`](LOCALIZATION_SEMANTIC_ISOLATION.md)。
